...
export class Graph {
    private adjacencyList: Map<string, { description: string, dependencies: string[], actors: string[] }>;
    ...
    public addVertex(vertex: string, description: string, actors: string[]): void {
        if (!this.adjacencyList.has(vertex)) {
            this.adjacencyList.set(vertex, { description, dependencies: [], actors });
        }
    }
    ...
    public createAnalysis(dependencies: Dependency[], nodes: Node[]): {containsCycle: string | null, topologicalSort: string, table: string} {
        const graph = new Graph();
        
        nodes.forEach(item => {
            // Pass actors along with node and description
            graph.addVertex(item.node, item.description, item.actors ?? []);
        });
        ...
        const containsCycle = graph.containsCycle();
